...
if (in_array($_SESSION['id'], $clicklike_mem_arr) || in_array($_SESSION['id'], $clickunlike_mem_arr)) {
    ...
    if (in_array($_SESSION['id'], $clicklike_mem_arr)) {
        ...
        $like = $row['clicklike'];
        $num = $like - 1;
        $sql = "UPDATE thumbsup SET clicklike ='".$num."', clicklike_mem='".$upda_str."' WHERE click_msg_id='".$_POST['messageid']."'";
        // echo $sql;
        $conn->query($sql);
        ...
        // echo $join_clicklike_mem;

        $like = $row['clicklike'];
        $num = $like + 1;
        $sql = "UPDATE thumbsup SET clicklike ='".$num."', clicklike_mem='".$join_clicklike_mem."' WHERE click_msg_id='".$_POST['messageid']."'";
        // echo $sql;
        $conn->query($sql);
        ...
        $sql2 = "UPDATE thumbsup SET clickunlike ='".$num."', clickunlike_mem='".$upda_str."' WHERE click_msg_id='".$_POST['messageid']."'";
        // echo $sql2;
        $conn->query($sql2);
    ...
    //如果按讚的會員有不在like陣列或unlike陣列裡面，則可按讚
    ...
    if($result->num_rows == 0){
        //如果此留言沒有被按讚的紀錄
        $sql = "INSERT INTO thumbsup (click_msg_id, clicklike, clicklike_mem) VALUES ('".$_POST['messageid']."', '1', '".$join_clicklike_mem."')";
        // echo $sql;
        $conn->query($sql);
    }else{
        //如果此留言有被按讚的紀錄
